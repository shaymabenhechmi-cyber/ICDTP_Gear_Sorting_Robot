...
/* ── Echo-Mode: test the full Pi↔STM32 communication chain without
 *    motor hardware / PCB.  Set to 0 when real motors are connected. ── */
#define ECHO_MODE 0
...
/* Status report timing: send every ~5ms using SysTick */
...
#if ECHO_MODE
/* ── Echo helpers: immediately copy target_steps into motor positions ── */
static void echo_apply_stream(void)
{
    for (int i = 0; i < 4; i++)
    {
        motors[i]->motion.position = target_steps[i];
        motors[i]->active_movement_flag = 0;
    }
}

static void echo_apply_reset(void)
{
    for (int i = 0; i < 5; i++)
    {
        motors[i]->motion.position = 0;
        motors[i]->active_movement_flag = 0;
        motors[i]->stallguard.stall_flag = 0;
    }
    homing_done_flag = 1;
    error_flag = 0;
}

static void echo_apply_home(void)
{
    for (int i = 0; i < 4; i++)
    {
        motors[i]->motion.position = home_steps[i];
        motors[i]->active_movement_flag = 0;
    }
}

static void echo_apply_gripper(uint8_t cmd)
{
    if (cmd == CMD_GRIP_OPEN)
    {
        motors[4]->motion.position = GRIPPER_STEPS_OPEN;
        motors[4]->stallguard.stall_flag = 0;
    }
    else if (cmd == CMD_GRIP_CLOSE)
    {
        motors[4]->motion.position = GRIPPER_STEPS_CLOSED;
        motors[4]->stallguard.stall_flag = 1;  // simulate gripped
    }
    motors[4]->active_movement_flag = 0;
}
#endif /* ECHO_MODE */
...
int main(void)
{
...
#if ECHO_MODE
  /* ── Echo-Mode: no motor init, no display, no button wait ─────────── */
  /* Zero out motor structs so fields are in a known state */
  ...
  /* Start listening for serial commands from Pi */
  startSerialReceive();

  /* Signal ready: toggle on-board LED (PA5) */
  HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_5);

#else
  /* ── Normal Mode: full motor initialisation ──────────────────────── */
  initAllMotors(MODE_5_UART);

  writeDisplay("Waiting for Pi...");

  /* Start listening for serial commands from Pi */
  startSerialReceive();

  /* Wait for blue button press as hardware safety interlock */
  // while(start_flag == 0);
  start_flag = 1;

  writeDisplay("Serial ready");
#endif /* ECHO_MODE */
  ...
  {
#if ECHO_MODE
	  /* ── Echo-Mode: no real motor movement, instant position update ── */
	  ...
	  if (gripper_cmd != 0)
	  {
		  uint8_t cmd = gripper_cmd;
		  gripper_cmd = 0;
		  echo_apply_gripper(cmd);
	  }

	  /* Periodic status report to Pi (~50ms) */
...
#else
	  /* ── Normal Mode: real motor control ─────────────────────────── */
	  ...
	  /* ── Reset / StallGuard Homing ─────────────────────────── */
	  static uint8_t is_homing_active = 0;
	  ...
	  /* If homing is active, we check if all motors have stopped */
	  if (is_homing_active)
	  {
		  ...
		  if (all_stopped) {
			  ...
			  homing_done_flag = 1;
			  for (int i = 0; i < 4; i++) {
				  motors[i]->motion.position = 0; // Reset position to 0 
                  motors[i]->motion.step = 0;
			  }
			  writeDisplay("Homing done");
		  } else {
			  // Ignore stream commands while homing is in progress
			  new_target_flag = 0;
			  home_request = 0;
		  }
	  }

	  /* If we haven't homed yet, ignore all incoming movement commands */
	  if (!homing_done_flag)
	  {
		  new_target_flag = 0;
		  home_request = 0;
	  ...
	  /* ── Home to predefined position ──────────────────────── */
	  if (home_request)
	  {
		  home_request = 0;
		  for (int i = 0; i < 4; i++)
			  moveToStep(home_steps[i], motors[i]);
	  }

	  /* ── Stream: update target positions for 4 arm axes ──── */
	  if (new_target_flag)
	  {
		  new_target_flag = 0;
		  for (int i = 0; i < 4; i++)
			  streamMoveToStep(target_steps[i], motors[i]);
	  }
	  ...
	  if (gripper_cmd != 0)
	  {
		  ...
		  gripper_cmd = 0;
		  if (cmd == CMD_GRIP_OPEN)
			  moveGripperToStep(GRIPPER_STEPS_OPEN);
		  else if (cmd == CMD_GRIP_CLOSE)
			  moveGripperToStep(GRIPPER_STEPS_CLOSED);
	  }

	  /* ── StallGuard monitoring for all active motors ──────── */
	  checkAllDrivers();
...
/* printf redirected to SWO (ITM port 0) instead of USART2.
 * USART2 is now reserved for the Pi serial protocol.
 * To view: use SWV ITM Data Console in STM32CubeIDE. */
PUTCHAR_PROTOTYPE
{
  ITM_SendChar(ch);
  return ch;
